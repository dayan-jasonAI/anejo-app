...
| Slots | Width at `min-width: 62px` | 375px viewport |
|---|---|---|
| 6 | 372px | fits |
| 7 | 434px | **overflows by 59px** |

`min-width` is **62px, not 64px** — `6 × 64 = 384` clipped the last slot (the `More` button on every
role) by 9px and made the bar scroll a hair. `6 × 62 = 372` fits with room to spare, and the tap
target is still 62×60, far above the 44×44 floor. Measured in a browser against the live renderers,
before and after — not derived.
...
- **kitchen / driver / shared pages** — `Hub.renderNavWithMore()` in `public/hub/assets/hub.js`.
...
- **320px (iPhone SE 1st gen) still scrolls** — 52px over. Six slots need ≥372px. Accepted: the
  field devices are modern phones and a kitchen tablet.
