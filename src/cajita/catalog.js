export const foods = [
  ...
  },
  {
    id: "grazing",
    name: "Grazing bites",
    detail: "A little sweet, a little savory.",
  },
  {
    id: "tres-leches",
    name: "Tres leches",
...
export function newVariant() {
  return {
    ...
    personalization: {
      labelText: "La Cajita",
      tagText: "Hecho con amor",
      pickText: "",
      artworks: [],
      textPlacements: ["label", "tag", "pick"].map((surface) => ({
        surface,
        x: 0.5,
        y: 0.83,
        scale: 1,
...
export function totals(config) {
  return {
    boxes: config.variants.reduce((n, v) => n + v.quantity, 0),
    items: Object.fromEntries(
      foods.map(({ id }) => [
        id,
        config.variants.reduce(
          (n, v) =>
            n + v.quantity * (v.items.find((i) => i.id === id)?.quantity || 0),
          0,
        ),
      ]),
    ),
