...
export async function onRequestGet({ request, env }) {
  ...
  return json({
    ...
    revenue_week_cents: await scalar(env, "SELECT COALESCE(SUM(total_estimate_cents),0) n FROM orders WHERE created_at>=? AND status IN ('paid','prep','ready','fulfilled')", weekAgo),
