...
export const onRequestPost = async ({ request, env }) => {
  ...
  switch (String((b && b.op) || '')) {
    case 'start_sequence': return reply(await startSequence(env, { opportunity_id: String(b.opportunity_id || ''), contact_id: b.contact_id || null, cfg, ctx, base }));
    case 'preview': return reply(await previewOutreach(env, String(b.id || ''), { cfg, subject: b.subject, body: b.body, base }));
    case 'approve': return reply(await approveOutreach(env, String(b.id || ''), {
      cfg, subject: b.subject, body: b.body, render_hash: b.render_hash, acknowledge_flags: b.acknowledge_flags === true, ctx, base,
    }));
    ...
    case 'send_now': return json(await sendApproved(env, { cfg, base, limit: 10 }));
