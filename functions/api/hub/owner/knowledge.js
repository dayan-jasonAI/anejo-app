...
export const onRequestGet = async ({ request, env }) => {
  ...
  try {
    ...
    // Say so. Swallowing this rendered "No documents yet" for a database failure — the owner would
    // conclude their library was empty and re-upload everything.
    items = [];
    loadError = (e && e.message) || 'Could not read the library.';
  }
...
export const onRequestPost = async ({ request, env }) => {
  ...
  if (dataUrl) {
    ...
    // PDFs go to Claude FIRST. Both cheaper paths failed on the first real document uploaded here
    // (DBPR Industry Bulletin 2026-01): toMarkdown returned the PDF's metadata with an empty
    // Contents section, and the raw stream extractor produced mojibake for every run set in a
    // subsetted font ("$GYHUWLVLQJ³IUHVK´" for "Advertising 'fresh'"). Since every DBPR bulletin is
    // generated by the same Word pipeline, that is the normal case for this library, not an edge
    // case — and half-garbled food-safety text is worse than none, because it still gets retrieved
    // and cited.
    if (isPdf) {
      markdown = await extractPdfWithClaude(env, decoded.bytes);
      if (markdown) extractor = 'claude';
    }

    // Non-PDFs (Word, spreadsheets, images) — and PDFs if Claude was unavailable — use toMarkdown.
    if (!markdown && env.AI) {
      try {
        ...
        const data = (first && first.format !== 'error' && first.data) || '';
        // A result whose Contents section is empty is a FAILED extraction wearing a success badge.
        // Accepting it is how the first upload here indexed nothing but PDF version numbers and
        // author names, and reported "1 of 1 passages indexed".
        if (data && contentAfterMetadata(data) >= 200) { markdown = data; extractor = 'tomarkdown'; }
      } catch { /* fall through to the stream extractor */ }
    }

    // Last resort for PDFs: read the content streams directly.
    if (!markdown && isPdf) {
      const raw = await extractPdfText(decoded.bytes);
      if (raw && !looksGarbled(raw)) { markdown = raw; extractor = 'pdf-streams'; }
    }

    if (!markdown) {
      return bad(
        'Could not read the text out of that file. If it is a scan or uses embedded fonts, ' +
        'open it, copy the text, and paste it in instead — an unreadable document must not be ' +
        'indexed, because half-extracted rules get quoted back as if they were complete.',
        422,
      );
    }

    // Drop toMarkdown's "## Metadata" property block. It is not knowledge, and it retrieved as the
    // top hit for a real question on the first upload — PDF producer strings competing with SOPs.
    const ci = markdown.search(/^##\s+Contents\s*$/mi);
    if (ci !== -1) markdown = markdown.slice(ci + 12).trim();
  }
  ...
  await capture(env, {
    ...
    properties: { doc_id: docId, source_kind: sourceKind, authority, chunks: chunks.length, indexed, failed, status, extractor },
  });
