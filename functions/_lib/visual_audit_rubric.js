...
export const FORMAT={type:'json_schema',schema:{type:'object',additionalProperties:false,required:['rubric_version','observations','suggestions','product_evidence'],properties:{
 ...
 product_evidence:{type:'object',additionalProperties:false,required:['scope','claims','unreadable_slides'],properties:{
  ...
  claims:{type:'array',description:'At most 6 explicit product/ingredient/quantity claims. Do not infer a SKU or ingredients from appearance.',items:{type:'object',additionalProperties:false,required:['caption_line','slide','quote'],properties:{caption_line:{type:'integer'},slide:{type:'integer'},quote:{type:'string',description:'Exact substring of caption line or visible overlay, at most250characters; never appearance guesses.'}}}},
...
export function validateVisualAudit(data,{caption,slideCount,brandText,brandReceipt,trainingReceipt,emblemReference,images=[]}){
 ...
 for(const claim of product.claims){
  if(!keys(claim,['caption_line','slide','quote']) || !Number.isInteger(claim.caption_line) || claim.caption_line<0 || claim.caption_line>lines.length || !Number.isInteger(claim.slide) || claim.slide<0 || claim.slide>slideCount || (!!claim.caption_line===!!claim.slide) || typeof claim.quote!=='string' || !claim.quote.trim() || claim.quote.length>250)return invalidTop('product_evidence','invalid_claim');
  ...
  if(facts && !facts.rendered_text.some(run=>run.text.includes(claim.quote)))return invalidTop('product_evidence','unsupported_registered_overlay_claim');
 }
