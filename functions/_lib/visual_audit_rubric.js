...
export function visualAuditFormat(caption, slideCount, authority={}) {
 ...
 // Anthropic supports required closed objects, but not exact-length arrays.
 // https://platform.claude.com/docs/en/build-with-claude/structured-outputs
 const observation=format.schema.properties.observations.items;
 delete observation.properties.criterion_id;
 observation.required=observation.required.filter(key=>key!=='criterion_id');
 format.schema.properties.observations={type:'object',additionalProperties:false,
  required:CRITERIA.map(c=>c.id),properties:Object.fromEntries(CRITERIA.map(c=>[c.id,structuredClone(observation)]))};
 return format;
}
...
// Only the current provider transport is accepted here. Saved/internal evidence remains an
// ordered array. Mapping supplies identity from an explicitly present key, never a finding.
export function validateVisualAuditTransport(data,context){
 const reject=(issue,extra={})=>fail('invalid_rubric_response',{reason:'invalid_rubric_response',field:'observations',issue,...extra});
 if(!plain(data)||!Object.hasOwn(data,'observations'))return validateVisualAudit(data,context);
 if(!plain(data.observations))return reject('keyed_object_required');
 const ids=CRITERIA.map(c=>c.id),observations=data.observations;
 const missing=ids.filter(id=>!Object.hasOwn(observations,id));
 const extra=Object.keys(observations).filter(key=>!ids.includes(key));
