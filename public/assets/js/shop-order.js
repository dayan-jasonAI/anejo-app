...
function renderShopChoices(){
 if(!shopActive)return;const format=$('shopFormat').value;
...
function shopChange(id,d){if(d>0&&!shopEnabled(PRICE[id]))return;shopDraft[id]=Math.max(0,Math.min(20-(shopEditing?0:(addons[id]||0)),(shopDraft[id]||0)+d));renderShopChoices();}
function applyShop(){
