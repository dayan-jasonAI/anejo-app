...
(function () {
  ...
  window.MarketingPhotoEnhance = {
    open: function (options) {
      ...
      var note = node('p', t('Photo polish adjusts the original pixels without inventing food or packaging. Optional AI retouch can change details and framing: compare both images carefully.', 'El ajuste fotográfico modifica los píxeles originales sin inventar comida ni empaque. El retoque opcional con IA puede cambiar detalles y encuadre: compara ambas imágenes.'));
      ...
      var methodLabel=node('label',t('Method','Método'));var method=node('select');method.setAttribute('aria-label',t('Photo method','Método de foto'));[['photographic','Photo polish — original pixels / Ajuste fotográfico'],['ai','AI retouch — experimental preview / Retoque IA experimental']].forEach(function(p){var o=node('option',p[1]);o.value=p[0];method.append(o);});methodLabel.append(method);
      ...
      var budget=node('p',t('Photo polish uses no AI credits. AI retouch uses the existing AI budget. Both save a separate labeled copy in Photos and preserve the original. Neither publishes a post. JPEG, PNG and WebP only; export camera RAW/HEIC first.', 'El ajuste fotográfico no usa créditos de IA. El retoque IA usa el presupuesto existente. Ambos guardan una copia identificada en Fotos y conservan el original. Ninguno publica. Solo JPEG, PNG y WebP; exporta RAW/HEIC primero.')); budget.className='hint';
      ...
      dialog.append(title,note,compare,methodLabel,label,generate,budget,status,review,use,close); document.body.append(dialog); dialog.showModal();
      ...
      return new Promise(function(resolve){
        ...
        generate.onclick=async function(){
          if(busy)return; busy=true; generate.disabled=true; preset.disabled=true; method.disabled=true; use.hidden=true; review.hidden=true; checkbox.checked=false;
          ...
          try {
            var isAI=method.value==='ai';
            var r=isAI ? await Hub.api('/api/hub/owner/marketing-photo-enhance',{method:'POST',body:{media_key:photo.media_key,preset:preset.value}}) : {ok:true,photo:await window.MarketingPhotoPolish.create(photo,preset.value)};
            ...
            compare.append(frame(isAI?t('AI-enhanced copy — review required','Copia mejorada con IA — requiere revisión'):t('Photo polish — original composition','Ajuste fotográfico — composición original'),result.url));
            ...
            review.hidden=!isAI; checkbox.checked=!isAI; use.hidden=false; use.disabled=isAI;
          } catch(e){if(!finished)status.textContent=e.message;}
          finally{busy=false;if(!finished){generate.disabled=false;preset.disabled=false;method.disabled=false;}}
        };
