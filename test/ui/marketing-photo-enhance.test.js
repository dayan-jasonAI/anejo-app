...
function harness(replies,onUse){
 const nodes=[],calls=[];let focused=false;
 function node(tag){const n={tag,children:[],checked:false,disabled:false,hidden:false,isConnected:true,listeners:{},setAttribute(k,v){this[k]=v;},append(...els){els.forEach(e=>{e.parent=this;this.children.push(e);});},get lastChild(){return this.children.at(-1);},remove(){this.isConnected=false;if(this.parent)this.parent.children=this.parent.children.filter(e=>e!==this);},showModal(){this.open=true;},close(){this.open=false;},addEventListener(k,v){this.listeners[k]=v;},focus(){focused=true;}};nodes.push(n);return n;}
 const ctx={document:{createElement:node,createTextNode:t=>({textContent:t}),body:node('body'),activeElement:{isConnected:true,focus(){focused=true;}}},window:{},Hub:{api:async(path,opts)=>{calls.push({path,opts});const r=replies.shift();return typeof r==='function'?r():r;}}};
 ...
 return{nodes,calls,promise,get focused(){return focused;},button(text){return nodes.find(n=>n.textContent===text);}};
}
