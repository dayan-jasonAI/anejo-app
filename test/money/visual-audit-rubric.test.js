...
const response=()=>({rubric_version:VERSION,product_evidence:{scope:'format_only_or_no_claim',claims:[],unreadable_slides:[]},observations:CRITERIA.map(c=>({criterion_id:c.id,status:'met',evidence_anchor:'slide:1',caption_line:0,slides:[1],explanation:'The inspected evidence satisfies this criterion.'})),suggestions:[]});
...
test('unsupported or incomplete observations cannot be accepted',()=>{
 for(const mutate of [d=>d.observations.pop(),d=>d.observations[0].criterion_id='invented',d=>d.observations[0].rule_quote='Invented narrower branding rule',d=>d.observations[0].caption_quote='An absent promise',d=>d.observations[0].slides=[4],d=>delete d.observations[0].evidence_anchor,d=>d.observations[0].status='not_applicable']){const d=response();mutate(d);assert.equal(check(d).available,false);assert.equal(check(d).score,null);}
});
