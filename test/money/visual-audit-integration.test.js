...
test('visual candidate sends rubric and retained coverage, no image persistence',async()=>{
 ...
 try{const r=await auditDraft(env,{caption:'Catering for your gathering',images:[{data:'test-image'}]});assert.equal(r.verdict,'pass');assert.deepEqual(body.output_config.format.schema.properties.observations.items.properties.caption_quote.enum,['','Catering for your gathering']);assert.equal(r.input_coverage.training.reads.rules,'empty');assert.equal(r.rubric_version,VERSION);assert.match(body.system,/VERSIONED VISUAL ACCEPTANCE/);assert.ok(!body.system.includes('Return ONLY JSON, nothing else:'));assert.equal(body.messages[0].content[2].source.data,'test-image');assert.equal(body.messages[0].content[4].source.media_type,'image/png');assert.equal(body.messages[0].content[1].text,'Slide 1 — COVER');assert.equal(r.input_coverage.emblem_reference.purpose,'visual_consistency_only');assert.ok(!env.DB.calls.some(c=>c.kind==='run'&&c.args.includes('test-image')));}finally{globalThis.fetch=original;}
});
...
test('actual visual request numbers publication JPEGs first and reference PNG last, never as cover',async()=>{
 ...
 globalThis.fetch=async(_,options)=>{request=JSON.parse(options.body);return {ok:true,json:async()=>({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify({rubric_version:VERSION,observations:CRITERIA.map(c=>({criterion_id:c.id,status:'met',caption_quote:'',slides:[1],explanation:'Test fixture evidence'})),suggestions:[]})}]})};};
