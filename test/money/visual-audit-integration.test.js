...
test('actual visual provider boundary rejects missing keys and legacy observation arrays',async()=>{
 const env=ownerEnv({ANTHROPIC_API_KEY:'test'});const original=globalThis.fetch;
 try {
  for(const mutate of [data=>delete data.observations.claims,data=>{data.observations=Object.entries(data.observations).map(([criterion_id,value])=>({criterion_id,...value}));}]){
   const data=answer();mutate(data);
   globalThis.fetch=async()=>({ok:true,json:async()=>({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify(data)}]})});
   const result=await auditDraft(env,{caption:'Catering',images:[{data:'fixture'}]});
   assert.equal(result.brand_score,null);assert.equal(result.verdict,'flag');
   assert.equal(result.audit_diagnostic.reason,'invalid_rubric_response');assert.equal(result.audit_diagnostic.field,'observations');
  }
...
test('known claims expand bounded request budget and validate the exact supplied menu authority',async()=>{
 ...
 globalThis.fetch=async(_,init)=>{body=JSON.parse(init.body);const data=answer();data.product_evidence={scope:'explicit_claims',claims:[{source:'registered_overlay',caption_line:0,slide:1,quote:'Croquetas',claim_id:'pc_test_0',authority_source:'menu',authority_quote:'This exact menu authority was never supplied'}],unreadable_slides:[]};return {ok:true,json:async()=>({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify(data)}]})};};
