...
const data=()=>({rubric_version:VERSION,product_evidence:{scope:'format_only_or_no_claim',claims:[],unreadable_slides:[]},observations:CRITERIA.map(c=>({criterion_id:c.id,status:'met',evidence_anchor:'slide:1',caption_line:0,slides:[1],explanation:'Visible evidence matches criterion.'})),suggestions:[]});
test('numeric caption contract resolves exact source casing and whitespace on server',()=>{
 ...
 const format=visualAuditFormat(caption,2).schema.properties.observations.properties.branding;
...
test('invalid caption IDs fail with bounded diagnostics, never normalize or fall back',()=>{
 ...
 assert.deepEqual(visualAuditFormat('',1).schema.properties.observations.properties.branding.properties.caption_line.enum,[0]);
});
